...
class OutlookAdaptor extends BaseAdaptor {
  ...
  executeJS () {
    if (LiveConfig.shimOutlookPDFPreview) {
      this[privPdfObserver] = new window.MutationObserver((mutations) => {
        const frames = document.body.querySelectorAll('object[type="application/pdf"]:not(.wavebox-pdf-iframe-preview-enabled)')
  ...
  */
  _renderWaveboxPDFPreview = (iframe) => {
    const src = iframe.getAttribute('data')
