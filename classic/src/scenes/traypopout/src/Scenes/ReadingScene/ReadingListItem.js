...
class ReadingListItem extends React.Component {
  ...
  static propTypes = {
    readingQueueItem: PropTypes.object.isRequired
  }
  ...
  componentWillReceiveProps (nextProps) {
    if (this.props.readingQueueItem.serviceId !== nextProps.readingQueueItem.serviceId) {
      this.setState(this.generateAccountState(nextProps.readingQueueItem.serviceId))
    }
  }
  ...
  state = (() => {
    return {
      ...this.generateAccountState(this.props.readingQueueItem.serviceId)
    }
  })()

  accountChanged = (accountState) => {
    this.setState(this.generateAccountState(this.props.readingQueueItem.serviceId, accountState))
  }
  ...
  */
  ...
  handleClick = (evt) => {
    if (this.props.onClick) {
      this.props.onClick(evt, this.props.readingQueueItem)
    }
  }
    ...
    return (
      <ListItem
        ...
        <div
          ...
          style={(readingQueueItem.favicon
            ? { backgroundImage: `url("${readingQueueItem.favicon}")` }
            : undefined)} />
        <ListItemText
          primary={(<div className={classes.primaryText}>{readingQueueItem.title}</div>)}
          disableTypography
          secondary={(
