...
const { CHROME_EXTENSION_INSTALL_PATH } = require('../../MProcManagers/PathManager')
const { CRExtension, CRExtensionI18n } = require('../../../shared/Models/CRExtension')
...
class CRExtensionFS {
  ...
  */
  static listInstalledExtensionIds () {
    try {
      return fs.readdirSync(CHROME_EXTENSION_INSTALL_PATH)
    } catch (ex) {
      return []
    }
  }
  ...
  */
  static readManifest (extensionId, versionString) {
    try {
      const manifest = fs.readJsonSync(path.join(CHROME_EXTENSION_INSTALL_PATH, extensionId, versionString, 'manifest.json'))
      return manifest
    } catch (ex) {
      return {}
    }
  ...
  */
  static listInstalledVersions (extensionId) {
    let versionStrings
    try {
      versionStrings = fs.readdirSync(path.join(CHROME_EXTENSION_INSTALL_PATH, extensionId))
    } catch (ex) {
      versionStrings = []
    }
  ...
  */
  static resolvePath (extensionId, versionString) {
    return path.join(CHROME_EXTENSION_INSTALL_PATH, extensionId, versionString)
  }
  ...
  */
  static loadTranslatedManifest (extensionId, versionString, locale) {
    const rootPath = path.join(CHROME_EXTENSION_INSTALL_PATH, extensionId, versionString)
  ...
  */
  static updateEntry (extensionId) {
    const rootPath = path.join(CHROME_EXTENSION_INSTALL_PATH, extensionId)
  ...
  */
  static setForRemoval (extensionId) {
    fs.writeFileSync(path.join(CHROME_EXTENSION_INSTALL_PATH, extensionId, UNINSTALL_FLAG), UNINSTALL_FLAG)
  }
}

module.exports = CRExtensionFS
